...
let shop = {
        ...
        */
        for (let i = 0; i < this.addToCartButtons.length; i++) {
            this.addToCartButtons[i].addEventListener('click', function () {
                ...
                // console.log(this.addToCartButtons[i].getAttribute("productId"));
    ...
    'submit': function () {
        ...
        let data = {
            ...
            'token': 'e91a4fdb0ec5e341da6ddfb2ea37cbf60811ee3fbb13bf4fced2cf9d837dbe14'
        };
        data = JSON.stringify(data);
        request.open("POST", this.urls.submit, true); // open() 還能接受第三個參數, 是用以表示這個 request 是否為 asynchronous (非同步) 的布林值, 預設為 true
